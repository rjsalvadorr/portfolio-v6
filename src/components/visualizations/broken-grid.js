...
class BrokenGridView extends React.Component {
  ...
  render() {
    ...
    if (!cGrid) {
      return <div className={vizStyles.bGridWrapper} ref={this.bGridRef}></div>
    }
    ...
    return (
      <div className="broken-grid-wrapper-wrapper">
        <div className={vizStyles.bGridWrapper} ref={this.bGridRef}>
          {this.state.circleGrid &&
            this.state.circleGrid.map((box, idx) => {
              return (
                <div
                  key={idx}
                  className={`box box--${box.id} ${vizStyles.bGridBox}`}
                ></div>
              )
            })}
        </div>
